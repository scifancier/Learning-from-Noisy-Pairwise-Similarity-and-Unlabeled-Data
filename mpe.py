...
def mpe(kernel,N,M,nu,lambda_upper_bound=8.,epsilon=0.04):
    ...
    distribution_RKHS_distance = sqrt(np.dot(dist_diff.T,
                                    np.dot(kernel, dist_diff))[0,0])
    lambda_left = 1 # N.B. heuristics added for SU class-prior estimatio
...
def wrapper(X_mixture,X_component,right):
    ...
    lambda_star_est_1=mpe(kernel,N,M,nu=nu1,lambda_upper_bound=lambda_right)
    kappa_star_est_1=(lambda_star_est_1-1)/lambda_star_est_1

    return (0,kappa_star_est_1)
